...
// 9. 깊은 복사의 필요성
    /*
    - 중첩된 객체 수정 : 중첩된 겍체를 수정할 때 원본 객체에 영향을 주지 않으려면 깊은 복사가 필요함.
...
// 19. `Symbol()`타입의 직렬화
    // `Symbol()`타입은 `JSON.strongfy()`에 의해 직렬화 되지 않는다.(무시된다)

// 20. `JSON.strongfy()`의 순환 참조 문제
    // 객체가 순환 참조(자기 자신을 참조)하는 경우 `JSON.strongfy()`는 `TypeError`를 발생시킨다.